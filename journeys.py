...
def get_distance(point1: dict, point2: dict) -> tuple:
    """
    Gets the driving distance and duration between two points using http://project-osrm.org/docs/v5.10.0/api/#nearest-service
...
def create_dist_array():
    ...
    Returns:
    - dist_array (list): A list of tuples containing the origin index, destination index, duration (in seconds), and distance (in meters) between each pair of points.
    """
    dist_array = []
...
def create_distances_df():
    ...
    Returns:
    - distances_df (pd.DataFrame): The DataFrame containing the distances between each pair of points, including origin and destination names, distance in miles, duration in HH:MM:SS format, and a fixture key.
    """
    
    distances_df = pd.DataFrame(dist_array,columns=["origin","destination","duration(s)","distance(m)"])
    distances_df = distances_df.merge(data[["team"]], left_on = "origin", right_index=True).rename(columns={"team":"origin_name"})
    distances_df = distances_df.merge(data[["team"]], left_on = "destination", right_index=True).rename(columns={"team":"destination_name"})
    ...
    distances_df['fixture_key'] = distances_df['destination_name'].str.strip() + '-' + distances_df['origin_name'].str.strip().astype(str)
